...
# Load environment variables for contract deployment.
# Default `load_dotenv()` only searches upward from the current working directory,
# so depending on where `algokit project deploy` is executed from, the correct `.env`
# file might not be discovered. We explicitly try likely locations.
load_paths: list[Path] = []
try:
    # 1) Contract project root: .../projects/farmer-pay-contract/.env
    load_paths.append(root_path.parent / ".env")
except Exception:
    pass

try:
    # 2) Repo backend env: .../backend/.env
    # smart_contracts/__main__.py => parents[0]=.../projects/farmer-pay-contract,
    # parents[1]=.../projects, parents[2]=.../farmer-pay-contract, parents[3]=.../PICT 2.0
    repo_root = root_path.parents[3]
    load_paths.append(repo_root / "backend" / ".env")
except Exception:
    pass
